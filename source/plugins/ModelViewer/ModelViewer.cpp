...
ModelViewer::ModelViewer()
{
    ...
    // ── Assign params to named groups ─────────────────────────────────────
    // Indices match the order params were added above, starting at 0.
    // If you add or remove params, update the indices here to match.
    auto group = [&]( unsigned int idx, const std::string& name )
    {
        SetParamGroup( idx, name );
    };

    group(  0, "File" );       // objPath
    group(  1, "File" );       // loadModel

    group(  2, "Transform" );  // rotateX
    group(  3, "Transform" );  // rotateY
    group(  4, "Transform" );  // rotateZ
    group(  5, "Transform" );  // scale
    group(  6, "Transform" );  // translateX
    group(  7, "Transform" );  // translateY

    group(  8, "Camera" );     // fov
    group(  9, "Camera" );     // camDist
    group( 10, "Camera" );     // orbitX
    group( 11, "Camera" );     // orbitY

    group( 12, "Lighting" );   // lightDirX
    group( 13, "Lighting" );   // lightDirY
    group( 14, "Lighting" );   // lightR
    group( 15, "Lighting" );   // lightG
    group( 16, "Lighting" );   // lightB
    group( 17, "Lighting" );   // ambient
    group( 18, "Lighting" );   // specular
    group( 19, "Lighting" );   // shininess

    group( 20, "Material" );   // baseR
    group( 21, "Material" );   // baseG
    group( 22, "Material" );   // baseB
    group( 23, "Material" );   // wireframe
    group( 24, "Material" );   // wireR
    group( 25, "Material" );   // wireG
    group( 26, "Material" );   // wireB
    group( 27, "Material" );   // fresnel
    group( 28, "Material" );   // fresnelR
    group( 29, "Material" );   // fresnelG
    group( 30, "Material" );   // fresnelB

    group( 31, "Vertex FX" );  // explode
    group( 32, "Vertex FX" );  // waveAmp
    group( 33, "Vertex FX" );  // waveFreq
    group( 34, "Vertex FX" );  // waveSpeed

    group( 35, "Render" );     // shadingMode
    group( 36, "Render" );     // blendMode

    group( 37, "Output" );     // bgOpacity
    group( 38, "Output" );     // bgR
    group( 39, "Output" );     // bgG
    group( 40, "Output" );     // bgB
}
